...
forest_exits = [locations[ways] for ways in exits.keys() for location in exits[ways].values() if location == 1]
print(forest_exits)

for keys in locations.keys():
    forest_exits = [(ways, locations[ways]) for ways in
                    exits.keys() for location in exits[ways].values() if location == keys]
    print(forest_exits)
